...
namespace LootMagnet
{
    ...
    public class ModConfig
    {
        ...
        [CustomSettings.LocalSettingDescription(Strings.Culture.CULTURE_EN_US, "Use improved quick sell UI. Allows to sell items from reward UI, salvage UI including units parts. Disables old 'shift+click' UI. To sell item from reward and salvage screen click item icon (if sell is possible icon will be changed on cursor hover). To sell one item from mechlab double-click item icon, to sell all items shift+double-click item icon")]
        [CustomSettings.LocalSettingDescription(Strings.Culture.CULTURE_RU_RU, "Улучшенный интерфейс продажи. Позволяет продавать предметы из интерфеса лутбоксов, сальваги, так же позволяет продавать части юнитов. Отключает старый интерфейс продажи по shift+click. Для продажи предметов из интерфейсов лутбоксов и слаьваги шелкните по иконке премета в его слоте (если продажа таким образом возможна, при наведении иконка изменит форму). Для продажи одного предмета из интерфейса мехлаба - двойной клик по иконке, что бы продать всю пачку shift+двойной клик")]
